...
pub(super) fn runtime_input_impl(index: i64) -> VmResult<i64> {
    with_context(|context| {
...
pub(super) fn runtime_arg_impl(index: i64) -> VmResult<String> {
    with_context(|context| {
...
pub(super) fn runtime_arg_int_impl(index: i64) -> VmResult<i64> {
    with_context(|context| {
        let index = non_negative_index(index, "argument index")?;
        let value = context
            .args
...
pub(super) fn runtime_arg_int_or_impl(index: i64, default: i64) -> VmResult<i64> {
    with_context(|context| {
        let index = non_negative_index(index, "argument index")?;
...
pub(super) fn runtime_arg_float_or_impl(index: i64, default: f64) -> VmResult<f64> {
    with_context(|context| {
        let index = non_negative_index(index, "argument index")?;
        let Some(value) = context.args.get(index) else {
            return Ok(default);
...
pub(super) fn runtime_arg_or_impl(index: i64, default: &str) -> VmResult<String> {
    with_context(|context| {
        let index = non_negative_index(index, "argument index")?;
        let value = context
            .args
...
pub(super) fn runtime_set_output_impl(tensor: i64) -> VmResult<bool> {
    with_context(|context| {
...
pub(super) fn runtime_set_text_output_impl(text: &str) -> VmResult<bool> {
    with_context(|context| {
...
pub(super) fn runtime_start_timer_impl() -> VmResult<bool> {
    with_context(|context| {
...
pub(super) fn runtime_start_decode_timer_impl() -> VmResult<bool> {
    with_context(|context| {
...
pub(super) fn runtime_set_token_count_impl(generated_tokens: i64) -> VmResult<bool> {
    with_context(|context| {
...
pub(super) fn runtime_set_decode_token_count_impl(decode_tokens: i64) -> VmResult<bool> {
    with_context(|context| {
        if decode_tokens < 0 {
...
pub(super) fn runtime_compact2_impl(first: i64, second: i64) -> VmResult<i64> {
    with_context(|context| {
